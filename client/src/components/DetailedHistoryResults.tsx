...
function FormulaCell({ formula, kind }: { formula?: DetailedFormula; kind: "history" | "next" }) {
  ...
  return <td data-testid={kind === "next" ? "next-prediction-cell" : undefined} className={`min-w-11 border-b border-[#eaecf0] px-2 py-2 text-center text-xs font-semibold leading-4 transition-colors ${className}`} title={label}>{prediction || "—"}</td>;
}
...
export function DetailedHistoryResults({ items, loading, customFormulas = [] }: { items: DetailedHistoryRow[]; loading: boolean; customFormulas?: CustomFormulaPreview[] }) {
  ...
  return <section className="mt-5" data-testid="detailed-history-results"><div className="mb-3 flex flex-wrap items-end justify-between gap-3"><div><p className="text-[11px] font-bold tracking-[0.14em] text-[#667085]">FORMULA HISTORY</p><p className="mt-1 text-sm font-semibold text-[#172033]">歷史資料結果</p><p className="mt-1 text-xs leading-5 text-[#667085]">紅色為歷史命中，藍色為歷史未命中；下一局只顯示預測，不參與命中統計。</p></div><p className="rounded-full bg-[#f2f4f7] px-3 py-1 text-xs font-semibold text-[#475467]">最近 {displayRows.length} 局</p></div><div className="overflow-x-auto rounded-[14px] border border-[#e4e7ec] bg-white shadow-[0_2px_8px_rgba(16,24,40,0.05)]"><table className="min-w-[1840px] border-collapse text-xs" aria-label="歷史資料公式預測表"><thead><tr className="bg-[#f8fafc] text-[#475467]">{["局數", "閒1", "閒2", "閒3", "莊1", "莊2", "莊3", "閒點", "莊點", "結果", ...FORMULA_ORDER].map(name => <th key={name} className="min-w-11 border-b border-[#eaecf0] px-2 py-2.5 text-center text-[11px] font-bold whitespace-nowrap">{name}</th>)}</tr></thead><tbody>{displayRows.map(row => <tr key={row.id ?? row.roundNo} className="bg-white text-[#344054] hover:bg-[#fcfcfd]"><td className="border-b border-[#eaecf0] px-2 py-2 text-center font-bold text-[#172033]">{row.roundNo}</td>{[0, 1, 2].map(index => <td key={`p-${index}`} className="min-w-11 border-b border-[#eaecf0] px-2 py-2 text-center font-medium">{cardLabel(row.playerCards[index])}</td>)}{[0, 1, 2].map(index => <td key={`b-${index}`} className="min-w-11 border-b border-[#eaecf0] px-2 py-2 text-center font-medium">{cardLabel(row.bankerCards[index])}</td>)}<td className="border-b border-[#eaecf0] px-2 py-2 text-center font-semibold">{row.playerPoint}</td><td className="border-b border-[#eaecf0] px-2 py-2 text-center font-semibold">{row.bankerPoint}</td><td className={`border-b border-[#eaecf0] px-2 py-2 text-center font-extrabold ${predictionTextClass(row.winner)}`}>{row.winner}</td>{FORMULA_ORDER.map(name => <FormulaCell key={name} formula={rowFormula(row, name)} kind="history" />)}</tr>)}<tr data-testid="next-prediction-row" className="bg-[#f2f4f7] text-[#344054]"><td colSpan={10} className="border-b border-[#eaecf0] px-3 py-2.5 text-left font-bold">下一局 · 第 {latest.roundNo + 1} 局預測<span className="ml-2 text-[11px] font-medium text-[#667085]">僅預測，不計入命中</span></td>{FORMULA_ORDER.map(name => <FormulaCell key={name} formula={rowFormula(latest, name)} kind="next" />)}</tr><tr data-testid="historical-hit-row" className="bg-[#f2f4f7] text-[#344054]"><td colSpan={10} className="px-3 py-2.5 text-left text-xs font-bold">已完成局數命中</td>{FORMULA_ORDER.map(name => { const value = historicalFormulaHits[name]; const className = value === highestHit && highestHit > 0 ? "text-[#b42318]" : value === lowestHit ? "text-[#175cd3]" : "text-[#344054]"; return <td key={name} className={`px-2 py-2.5 text-center text-xs font-extrabold ${className}`}>{value}</td>; })}</tr></tbody></table></div><CustomFormulaSection formulas={customFormulas} /></section>;
}
